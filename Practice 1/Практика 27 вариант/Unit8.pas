...
function count(l:integer):integer;
...
begin
  ...
  min:=lines[1].Get_length;
  max:=lines[1].Get_length;
  for i:=1 to n do begin
    if lines[i].Get_length<min then
      min:=lines[i].Get_length;
    if lines[i].Get_count>max then
      max:=lines[i].Get_length;
  end;
...
begin
  for i:=1 to n do 
    if lines[i].Get_length=l then result:=lines[i].Get_count;
end;
